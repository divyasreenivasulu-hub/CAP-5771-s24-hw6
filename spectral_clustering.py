...
def compute_SSE(data, labels):
    ...
    for i in np.unique(labels):
        cluster_dat_points = data[labels == i]
        cluster_dat_center = np.mean(cluster_dat_points, axis=0)
        sse += np.sum((cluster_dat_points - cluster_dat_center) ** 2)
    return sse
...
def spectral_hyper_parameter(data, labels):
    """
...
def spectral_clustering():
    ...
    sigmas, ari_scores, sse_scores = spectral_hyper_parameter(data_subset, labels_subset)
    ...
    plt.title(f'Clustering for Dataset {best_dataset_index} (largest ARI) with k value')
    ...
    plt.title(f'Clustering for Dataset {best_dataset_index_sse} (Least SSE) with k value')
    plt.suptitle('Spectral Clustering')
    plt.xlabel('Feature 1')
    plt.ylabel('Feature 2')
    plt.colorbar(label='Cluster')
